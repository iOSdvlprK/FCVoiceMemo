...
struct VoiceRecorderView: View {
    ...
    var body: some View {
        ...
        .alert("선택된 음성메모를 삭제하시겠습니까?", isPresented: $voiceRecorderViewModel.isDisplayRemoveVoiceRecorderAlert) {
            ...
            Button(action: {
                ...
                VStack {
                    ...
                    HStack {
                        ...
                        if voiceRecorderViewModel.selectedRecordedFile != recordedFile, let duration {
    ...
    fileprivate var body: some View {
        GeometryReader { geometry in
            ZStack(alignment: .leading) {
                ...
                Rectangle()
                    .fill(.customGreen)
                // Clamp progress to [0, 1] and ensure it is a finite number
                    .frame(width: max(0, min(1, CGFloat(self.progress.isNaN ? 0 : self.progress))) * geometry.size.width)
            }
        }
    }
}
